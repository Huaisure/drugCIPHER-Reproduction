...
from utils import load_fingerprint, cal_test_ts, cal_test_closeness, shortest_path_length_dict
...
for j in range(len(test)):
    ...
    targets_closeness = get_closeness_lhs(test_dg_id)
    concordance_score_ts_list.append([abs(np.corrcoef(drugTS_list, target_closeness)[0, 1]) for target_closeness in targets_closeness])
...
for i in range(len(test)):
    ...
    rho_T_pd = concordance_score_ts_list[i]
    ...
    for target_closeness in targets_closeness:
        ...
        concordance_score_cs_list.append(list(concordance_score_cs))
        
        X = np.column_stack((np.array([test_ts]), np.array([test_cs]), np.ones_like(x)))
        beta = np.linalg.inv(X.T @ X) @ X.T @ np.array([targets_closeness])
        a_pd, b_pd, c = beta
        concordance_score_ms = abs(a * test_ts + b * test_cs + c - targets_closeness)
